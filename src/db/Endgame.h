...
//        ASSERT(checkNPieces(pieces1) || checkNPieces(pieces2));
    ...
    static int getEndgameValue(const int side,const ChessBoard::_Tboard &structureEval, _Tchessboard &chessboard) {
        const int nPieces = bitCount(structureEval.allPieces);
            ...
            case 4 :
                if (chessboard[QUEEN_BLACK]) {
                    if (chessboard[PAWN_WHITE]) {
                        ...
                        int result = KQKR(structureEval.posKing[BLACK], structureEval.posKing[WHITE], chessboard);
                        return side == BLACK ? result : -result;
                    }
                    ...
                    if (chessboard[PAWN_BLACK]) {
                        ...
                        int result = KQKR(structureEval.posKing[WHITE], structureEval.posKing[BLACK], chessboard);
                        return side == WHITE ? result : -result;
                    }
                } else if (chessboard[ROOK_BLACK]) {
                    if (chessboard[PAWN_WHITE]) {
